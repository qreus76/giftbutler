...
export async function GET(req: NextRequest, { params }: { params: Promise<{ username: string }> }) {
  ...
  // Deduplicate visits: only record once per session per 24 hours
  const visitorSession = req.headers.get("x-forwarded-for")?.split(",")[0].trim()
