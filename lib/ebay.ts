...
// Keys are pending verification — this module is ready to activate
...
export interface EbayMarketSummary {
  ...
  highestActiveListing: number;
  recentSoldCount: number;
  dealScore: "great" | "good" | "fair" | "overpriced";
}
...
export async function searchEbayListings(
  ...
  const res = await fetch(
    ...
    {
      headers: {
        Authorization: `Bearer ${await getEbayToken(appId)}`,
...
export async function getEbaySoldSummary(
  ...
  // Sold listings via Finding API (completed items)
  const params = new URLSearchParams({
  ...
  return {
    ...
    lowestActiveListing: lowest,
    highestActiveListing: highest,
